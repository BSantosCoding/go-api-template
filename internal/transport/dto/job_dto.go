...
import (
	"go-api-template/ent/job" // Import models for enums
	"time"
...
type ListJobsByEmployerRequest struct {
	...
	Limit      int        `form:"limit,default=10"`
	Offset     int        `form:"offset,default=0"`
	State      *job.State `form:"state" validate:"omitempty,oneof=Waiting Ongoing Complete Archived"`
	MinRate    *float64   `form:"min_rate" validate:"omitempty,gt=0"`
	MaxRate    *float64   `form:"max_rate" validate:"omitempty,gt=0,gtefield=MinRate"`
}
...
type UpdateJobRequest struct {
	...
	State        *job.State `json:"state,omitempty" validate:"omitempty,oneof=Waiting Ongoing Complete Archived"`
...
type UpdateJobStateRequest struct {
	State  job.State `json:"state" validate:"required,oneof=Waiting Ongoing Complete Archived"`
	JobID  uuid.UUID `json:"-"` // Set internally by handler from auth context
	UserID uuid.UUID `json:"-"` // Set internally by handler from auth context
}
