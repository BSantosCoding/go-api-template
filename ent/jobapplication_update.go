...
func (jau *JobApplicationUpdate) SetState(j jobapplication.State) *JobApplicationUpdate {
	jau.mutation.SetState(j)
	return jau
}

// SetNillableState sets the "state" field if the given value is not nil.
func (jau *JobApplicationUpdate) SetNillableState(j *jobapplication.State) *JobApplicationUpdate {
	if j != nil {
		jau.SetState(*j)
	}
	return jau
...
func (jau *JobApplicationUpdate) check() error {
	...
	if jau.mutation.ContractorCleared() && len(jau.mutation.ContractorIDs()) > 0 {
		return errors.New(`ent: clearing a required unique edge "JobApplication.contractor"`)
	}
...
func (jau *JobApplicationUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := jau.mutation.State(); ok {
...
// SetState sets the "state" field.
func (jauo *JobApplicationUpdateOne) SetState(j jobapplication.State) *JobApplicationUpdateOne {
	jauo.mutation.SetState(j)
	return jauo
}

// SetNillableState sets the "state" field if the given value is not nil.
func (jauo *JobApplicationUpdateOne) SetNillableState(j *jobapplication.State) *JobApplicationUpdateOne {
	if j != nil {
		jauo.SetState(*j)
	}
	return jauo
...
func (jauo *JobApplicationUpdateOne) check() error {
	if v, ok := jauo.mutation.State(); ok {
		if err := jobapplication.StateValidator(v); err != nil {
			return &ValidationError{Name: "state", err: fmt.Errorf(`ent: validator failed for field "JobApplication.state": %w`, err)}
		}
	}
	if jauo.mutation.ContractorCleared() && len(jauo.mutation.ContractorIDs()) > 0 {
		return errors.New(`ent: clearing a required unique edge "JobApplication.contractor"`)
	}
...
func (jauo *JobApplicationUpdateOne) sqlSave(ctx context.Context) (_node *JobApplication, err error) {
	...
	if value, ok := jauo.mutation.State(); ok {
		_spec.SetField(jobapplication.FieldState, field.TypeEnum, value)
	}
