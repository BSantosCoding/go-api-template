...
func (jaq *JobApplicationQuery) QueryContractor() *UserQuery {
...
func (jaq *JobApplicationQuery) Clone() *JobApplicationQuery {
	...
	return &JobApplicationQuery{
		...
		withContractor: jaq.withContractor.Clone(),
...
func (jaq *JobApplicationQuery) WithContractor(opts ...func(*UserQuery)) *JobApplicationQuery {
	query := (&UserClient{config: jaq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	jaq.withContractor = query
	return jaq
}
...
//		GroupBy(jobapplication.FieldJobID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.JobApplication.Query().
//		Select(jobapplication.FieldJobID).
//		Scan(ctx, &v)
...
func (jaq *JobApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*JobApplication, error) {
	...
	if query := jaq.withContractor; query != nil {
		if err := jaq.loadContractor(ctx, query, nodes, nil,
			func(n *JobApplication, e *User) { n.Edges.Contractor = e }); err != nil {
			return nil, err
		}
	}
...
func (jaq *JobApplicationQuery) loadContractor(ctx context.Context, query *UserQuery, nodes []*JobApplication, init func(*JobApplication), assign func(*JobApplication, *User)) error {
...
func (jaq *JobApplicationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := jaq.ctx.Fields; len(fields) > 0 {
		...
		if jaq.withContractor != nil {
			_spec.Node.AddColumnOnce(jobapplication.FieldContractorID)
		}
		if jaq.withJob != nil {
