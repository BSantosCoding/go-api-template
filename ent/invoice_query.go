...
func (iq *InvoiceQuery) QueryJob() *JobQuery {
	query := (&JobClient{config: iq.config}).Query()
...
func (iq *InvoiceQuery) WithJob(opts ...func(*JobQuery)) *InvoiceQuery {
	query := (&JobClient{config: iq.config}).Query()
...
func (iq *InvoiceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invoice, error) {
	...
	if query := iq.withJob; query != nil {
		if err := iq.loadJob(ctx, query, nodes, nil,
			func(n *Invoice, e *Job) { n.Edges.Job = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InvoiceQuery) loadJob(ctx context.Context, query *JobQuery, nodes []*Invoice, init func(*Invoice), assign func(*Invoice, *Job)) error {
...
func (iq *InvoiceQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := iq.ctx.Fields; len(fields) > 0 {
		...
		if iq.withJob != nil {
			_spec.Node.AddColumnOnce(invoice.FieldJobID)
		}
	}
